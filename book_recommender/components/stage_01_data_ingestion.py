...
class DataIngestion:
    ...
    def download_data(self) -> str:
        ...
        try:
            ...
            genre_slug = self.data_ingestion_config.genre_dataset_url
            ...
            genre_file_path = os.path.join(zip_download_dir, f"{genre_slug.split('/')[-1]}.zip")
            ...
            subprocess.run(["kaggle", "datasets", "download", "-d", [dataset_slug, genre_slug], "-p", zip_download_dir, "--force"], check=True)
            subprocess.run(["kaggle", "datasets", "download", "-d", genre_slug, "-p", zip_download_dir, "--force"], check=True)
            logging.info(f"Zipped Dataset Downloaded to: {zip_file_path}\n Genre Dataset Downloaded to: {genre_file_path}")
